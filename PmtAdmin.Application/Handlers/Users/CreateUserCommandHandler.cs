    ...
    public class CreateUserCommandHandler : IRequestHandler<CreateUserCommand, ApiResponse<List<UserDto>>>
    {
        ...
        public async Task<ApiResponse<List<UserDto>>> Handle(CreateUserCommand request, CancellationToken cancellationToken)
        {
            ...
            foreach (var userDto in request.Users)
            {
                ...
                {
                    // Track per-user creation error and continue with others
            ...
            // Map to DTOs; if mapping not configured, create a simple projection
            var mapped = _mapper.Map<List<UserDto>>(createdUsers);
            var userDtos = mapped ?? createdUsers.Select(u => new UserDto
            {
                Id = u.Id,
                Name = u.Name,
                Email = u.Email,
            ...
            if (errors.Any() && createdUsers.Any())
            {
                return new ApiResponse<List<UserDto>>(207, userDtos, message);
            }
